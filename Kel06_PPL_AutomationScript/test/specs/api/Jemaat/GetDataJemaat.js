...
describe('Get By ID rpp API Test', () => {
  const id = 3;
  const url = `http://127.0.0.1:8070/api/jemaat/${id}`; // Corrected URL with backticks for string interpolation

  it('should return a 200 status code', async () => {
    try {
      const response = await axios.get(url);
      expect(response.status).to.equal(200); // Check for a 200 status
    } catch (error) {
  ...
  it('should return response in json content-type', async () => {
    try {
      ...
      expect(contentType.toLowerCase()).to.include('application/json'); // Check if it's JSON or not
    } catch (error) {
