...
describe('Get All Baptis API Test', () => {
  const url = 'http://127.0.0.1:8070/api/jemaat';
  ...
  it('should return the response property "code" with 200 value', async () => {
    try {
      const response = await axios.get(url);
      // Check each item in the array for the 'code' property
      response.data.forEach(item => {
        expect(item).to.have.property('code').to.equal('200');
      });
    } catch (error) {
      throw new Error(`Failed to fetch todos: ${error.message}`);
    }
  ...
  it('should return the response property "message" with "Success" value', async () => {
    try {
      const response = await axios.get(url);
      // Check each item in the array for the 'message' property
      response.data.forEach(item => {
        expect(item).to.have.property('message').to.equal('Success');
      });
    } catch (error) {
      throw new Error(`Failed to fetch todos: ${error.message}`);
    }
  ...
  it('should return the response property "data" as an array', async () => {
    try {
      const response = await axios.get(url);
      expect(response.data).to.be.an('array'); // Check if the "data" property is an array
    } catch (error) {
      throw new Error(`Failed to fetch todos: ${error.message}`);
    }
  });
