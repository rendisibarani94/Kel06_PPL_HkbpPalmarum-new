...
describe("My Login application", () => {
  ...
  it("should search data in jadwal ibadah table and click the delete button", async()=> {

    const searchInput = $('.form-control.form-control-sm');
    searchInput.setValue('Minggu Trinitatis');
    await searchInput.click();

    await searchInput.setValue('Minggu Trinitatis');

    await browser.pause(2000);

    searchInput.keys('Enter');

    const editJemaat = $('.btn.btn-danger');
